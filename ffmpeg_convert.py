...
Da izveš, koliko video, audio in subtitle streamov ima neka datoteka, lahko uporabiš ffprobe v JSON načinu, podobno kot to že počneš za en sam audio stream.

🔧 Primer: pridobiš seznam vseh streamov
python
Copy
Edit
import json
...
🔄 Uporaba:
V funkciji convert_file(...) lahko potem zapišeš:

python
Copy
Edit
streams = get_streams(input_path)
video_count = len(streams["video"])
audio_count = len(streams["audio"])
subtitle_count = len(streams["subtitle"])
S tem lahko dinamično zgradiš -map in -c:<stream> za:

0:v:0 → konverzija

0:v:1, 0:v:2... → kopiranje
